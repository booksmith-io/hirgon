...
describe('Systemdata Model', () => {
  ...
  describe('constructor', () => {
    it('should initialize with table name "systemdata"', () => {
      expect(systemdata._table).toBe('systemdata');
    });
  });
  ...
  describe('remove method', () => {
    it('should call database delete with correct parameters', async () => {
      ...
      const mockDb = {
        where: jest.fn().mockReturnThis(),
        del: jest.fn().mockResolvedValue(expectedResult)
      };
      systemdata.dbh = () => mockDb;
  ...
  describe('get_format_systemdata method', () => {
    it('should return formatted systemdata object', async () => {
      ...
      const mockDb = {
        where: jest.fn().mockReturnThis(),
        select: jest.fn().mockResolvedValue(mockRawData)
      };
      systemdata.dbh = () => mockDb;
...
it('should preserve data types correctly', async () => {
      ...
      expect(mockDb.where).toHaveBeenCalledWith({});
      expect(mockDb.select).toHaveBeenCalledWith('*');
      expect(result['test:string']).toEqual({
        value: 'test-value',
        data: 'test-data',
        created_at: '2025-01-01 00:00:00',
        updated_at: '2025-01-01 00:00:00'
      });
      expect(typeof result['test:string'].data).toBe('string');
    });
    ...
    it('should handle null data gracefully', async () => {
      ...
      expect(mockDb.where).toHaveBeenCalledWith({});
      expect(mockDb.select).toHaveBeenCalledWith('*');
      expect(result['test:null']).toEqual({
        value: 'test-value',
        data: null,
        created_at: '2025-01-01 00:00:00',
        updated_at: '2025-01-01 00:00:00'
      });
      expect(result['test:undefined']).toEqual({
        value: 'test-value',
        data: undefined,
        created_at: '2025-01-01 00:00:00',
        updated_at: '2025-01-01 00:00:00'
    ...
    it('should handle edge cases', async () => {
      ...
      expect(result['test:empty']).toEqual({
        value: '',
        data: '',
        created_at: '2025-01-01 00:00:00',
        updated_at: '2025-01-01 00:00:00'
