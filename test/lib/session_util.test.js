...
describe('Session Utils', () => {
  ...
  describe('get_alert', () => {
    it('should return and remove alert from session', () => {
      ...
      expect(result).toEqual({ type: 'info', message: 'Test alert' });
      expect(mockReq.session.alert).toBeUndefined();
      expect(mockReq.session.user).toEqual({ id: 1 });
    });
  ...
  describe('set_alert', () => {
    it('should set alert in session', () => {
      ...
      expect(mockReq.session.alert).toEqual(alertData);
      expect(mockReq.session.user).toEqual({ id: 1 });
    });
    ...
    it('should override existing alert', () => {
      ...
      expect(mockReq.session.alert).toEqual(newAlertData);
    });
  });
  ...
  describe('build_config', () => {
    ...
    it('should build config for development environment', () => {
      ...
      expect(result.secret).toBe('dev-secret');
      expect(result.cookie.secure).toBe(false);
      expect(result.resave).toBe(false);
      expect(result.saveUninitialized).toBe(false);
      expect(result.cookie.maxAge).toBe(259200000);
    });
